...
Current reproducible pitch coverage is **28 of 1,094 primary matches (2.55941%)**.
All 28 are AI-assisted first-pass codes from non-ESPN, match-specific pre-match
...
The separate tracked verified release contains 28 cited scheduled starts, one for
...
powerplay columns. The first four balanced batches reviewed 100 matches: 28 passed,
while 72 are explicitly listed in `data/manual/pitch_set_aside.csv` for later work.
This 28% yield among reviewed matches is not representative of the full cohort;
