...
| 07 | outcome-blind eligible match queue | generate unfetched probable ESPN-ID links; manually verify identity, locate eligible pre-match reports, and code with the frozen codebook | queue + local `pitch_reports` working file | zero automated ESPN requests; candidate status explicit; source time precedes start; double-code ≥20% |
| 08 | primary-cohort metadata + cited schedules | generate an outcome-blind start-time reference; manually verify match identity, scheduled local time, and IANA timezone only for matches with collected pitch reports; convert to UTC | tracked `match_start_times_template` + ignored verified working subset + audit | working rows are in cohort; every verified pitch report has timing evidence; generated template covers full cohort; zero network requests by pipeline; exact IANA local-to-UTC conversion; timing fields prohibited from models |
