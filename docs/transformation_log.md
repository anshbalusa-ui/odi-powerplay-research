...
| 08 | primary-cohort metadata + cited schedules | generate an outcome-blind start-time queue; manually verify match identity, scheduled local time, and IANA timezone; convert to UTC | tracked `match_start_times_template` + ignored verified working file + audit | full cohort coverage; zero network requests by pipeline; valid provenance; exact IANA local-to-UTC conversion; only `verified` rows exposed downstream |
