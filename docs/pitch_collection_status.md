...
balanced 25-match batches and five 25-match batches selected newest-first have now
been reviewed: 65 non-ESPN reports passed source, timing, and coding validation,
while 160 matches were set aside with explicit reasons.
...
| Matches reviewed in nine 25-match batches | 225 |
| Timing-verified, source-coded matches | 65 |
| Matches set aside for later review | 160 |
| Leakage-safe model-table merge dry run | 65 |
| Current cohort coverage | 5.94150% |

The tracked `data/manual/pitch_reports_verified.csv` and
`data/manual/match_start_times_verified.csv` files contain the 65 verified rows.
...
for 160 reviewed matches without currently eligible analysis.
...
tracked `data/manual/match_start_times_verified.csv` release contains only the 65
