...
- A tracked seven-batch release with 49 timing-verified provisional pitch codes
  and 126 outcome-blind set-aside rows for later review.
...
The first four balanced pitch batches plus three newest-first batches now cover 175
reviewed matches: 49 non-ESPN pre-match reports passed source and timing validation,
and 126 matches are listed in `data/manual/pitch_set_aside.csv` for later review.
Current verified coverage is 49/1,094 (4.47898%). The minimized tracked releases are
